...
let burstFailed = 0;
// 本轮是否因令牌失效暂停过队列（是则排空时不再弹通用失败汇总）
let burstAuthPaused = false;
// 令牌失效提示的节流：并发中的多个任务几乎同时失败，只提示一次
let lastAuthNoticeAt = 0;

// 识别令牌失效类错误：课件/教材下载给出 Access Token 文案，m3u8 给出 HTTP 401/403
...
// 令牌失效时把仍在排队的任务全部挂起，避免连环失败刷屏；换令牌后可逐个/批量继续
function pauseQueueForAuthFailure(): void {
  burstAuthPaused = true;
  ...
  const now = Date.now();
  if (now - lastAuthNoticeAt > 5000) {
    lastAuthNoticeAt = now;
    ElMessage.error({
      message: 'Access Token 已失效，剩余任务已暂停。请到「设置」更新令牌后，在下载管理中继续。',
      duration: 6000,
    });
  }
}
...
async function startTask(task: DownloadTask): Promise<void> {
  ...
  try {
    ...
    if (currentStatus(task) === 'paused') {
      ...
      task.completedAt = Date.now();
      burstFailed += 1;
      // 令牌失效会让后续任务全部失败，第一时间挂起整个队列
      if (isAuthError(task.error)) {
        pauseQueueForAuthFailure();
      }
    }
  } finally {
    task.speed = 0;
...
function pump(): void {
  ...
  if (!hasQueued && inflight.size === 0 && burstStarted > 0) {
    ...
    const authPaused = burstAuthPaused;
    burstStarted = 0;
    burstCompleted = 0;
    burstFailed = 0;
    burstAuthPaused = false;

    if (failed > 0) {
      // 令牌失效场景已单独提示并暂停队列，不再叠加通用失败汇总
      if (!authPaused) {
